...
function ClientPlanDetails() {
  ...
  {Object.entries(
    ...
    (sum, meal) =>
        sum + meal.foods.reduce(
            (foodSum, food) =>
                foodSum + food.grams * food.food_norm?.calories_per_g, 0), 0
        );
    ...
    const totalProtein = dayMeals.reduce(
        (sum, meal) => 
            sum + meal.foods.reduce(
                (foodSum, food) => 
                    foodSum + food.grams * food.food_norm?.protein_per_g, 0), 0
        );
    const totalCarbs = dayMeals.reduce(
        (sum, meal) =>
            sum + meal.foods.reduce(
                (foodSum, food) =>
                    foodSum + food.grams * food.food_norm?.carbs_per_g, 0), 0
    );
    const totalFat = dayMeals.reduce(
        (sum, meal) =>
            sum + meal.foods.reduce(
                (foodSum, food) => 
                    foodSum + food.grams * food.food_norm?.fat_per_g, 0), 0
            );
                ...
                meal.foods.map((food) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-[#6b7280]">
                      {food.grams} g ·{" "}
                      {(food.grams * food.food_norm.calories_per_g).toFixed(0)} kcal
                    </p>

                    <p className="mt-1 text-xs text-[#9ca3af]">

                        {(food.grams * food.food_norm?.protein_per_g).toFixed(0)}P ·{" "}
                        {(food.grams * food.food_norm?.carbs_per_g).toFixed(0)}C ·{" "}
                        {(food.grams * food.food_norm?.fat_per_g).toFixed(0)}F
